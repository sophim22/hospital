										...
										<tbody>
											...
											?>
												...
												<tr>
													...
													<td class="hidden-xs">
														<?php
														$id = $row['specialization'];
														$ret = mysqli_query($con, "SELECT specialization FROM doctorspecialization WHERE id='$id'");

														if ($ret && mysqli_num_rows($ret) > 0) {
															$specialization = mysqli_fetch_assoc($ret);
															echo htmlspecialchars($specialization['specialization']);
														} else {
